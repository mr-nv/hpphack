...
{
	const auto ColorWindowBg = ImGui::GetStyleColorVec4(ImGuiCol_ChildBg);
	ImGui::PushStyleColor(ImGuiCol_Text, ColorWindowBg);
	const auto ret = ButtonEx(label, size_arg, 0);
	ImGui::PopStyleColor();
	return ret;
}
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 & pos, float radius)
{
	...
	if (hovered)
		window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius), GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered), 25);
...
void ImGui::NextLine(int spacing_count, float offset_from_start_x)
{
	Spacing(spacing_count);

	if (offset_from_start_x != -1.f)
...
void ImGui::Separator(float size)
{
	...
	float x2 = window->Pos.x + size ? size : window->Size.x;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2 & size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if (render_cursor || render_selection)
	{
...

		{
			...
			cursor_offset.y = searches_result_line_no[0] * (g.FontSize + 2.f);
			if (searches_result_line_no[1] >= 0)
			{
				select_start_offset.x = InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x;
				select_start_offset.y = searches_result_line_no[1] * (g.FontSize + 2.f);
			}

			// Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1224)
			if (is_multiline)
				text_size = ImVec2(size.x, line_count * (g.FontSize + 2.f));
		}
		...
		if (render_cursor && state->CursorFollow)
		{
			...
			if (is_multiline)
			{
				...
				if (cursor_offset.y - (g.FontSize + 2.f) < scroll_y)
					scroll_y = ImMax(0.0f, cursor_offset.y - (g.FontSize + 2.f));
		...
		if (render_selection)
		{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - (g.FontSize + 2.f)), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
					rect.ClipWith(clip_rect);
					if (rect.Overlaps(clip_rect))
						draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color);
				}
				rect_pos.x = draw_pos.x - draw_scroll.x;
				rect_pos.y += g.FontSize + 2.f;
			}
		}
		...
		if (render_cursor)
		{
			...
			ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - (g.FontSize + 2.f) + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
				draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));

			// Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
			if (!is_readonly)
				g.PlatformImePos = ImVec2(cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - (g.FontSize + 2.f));
		}
	}
	else
	{
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2 & size_arg)
{
	...
	{
		const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
		RenderFrame(bb.Min, bb.Max, col, false, 6.0f);
...
void ImGui::DrawPoint(float angle, float radius, ImVec2 pos, float size, const ImColor & col)
{
	pos.x = pos.x + ((pos.x - radius) - pos.x) * cosf(angle) - ((pos.y - radius) - pos.y) * sinf(angle);
	pos.y = pos.y + ((pos.y - radius) - pos.y) * cosf(angle) + ((pos.x - radius) - pos.x) * sinf(angle);

	ImGuiWindow * window = ImGui::GetCurrentWindow();
	window->DrawList->AddCircleFilled(pos, size, col);
}

void ImGui::Loading(const ImVec2 & pos, float size, const ImVec4 & col, float curtime)
{
	static auto init_time = curtime;
	const auto angle = IM_PI * 0.25f + (curtime - init_time) * IM_PI;
	for (int i = 0; i < 8; i++)
		DrawPoint(angle + 0.78f * (float)i, size, pos, size / 3.3f, col);
}
...
void ImGui::TextCenter(const char* fmt, ...)
{
	...
	const auto text_size = pFont->CalcTextSizeA(pFont->FontSize, FLT_MAX, 0, buf);
