...
namespace ImGui
{
	...
	static bool             BeginChildElementEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
...
void ImGui::UpdateMouseWheel()
{
	...
	static bool  bSmoothScrolling, bInterpolated;
	static float flInterolateTime, flScrollStart, flScrollEnd;
	...
	{
		flInterolateTime = g_pGlobalVars->curtime;
	...
	if (g.HoveredWindow && !g.HoveredWindow->Collapsed && bSmoothScrolling)
	{
		...
		else if (!g.IO.KeyCtrl && scroll_allowed)
		{
			...
			auto fInterp = [&](float s1, float s2, float s3, float f1, float f3) -> float
			{
				if (s2 == s1)
					return f1;
			...
			float flScrollLepred = fInterp(flInterolateTime, g_pGlobalVars->curtime,
				flInterolateTime + 0.125f, flScrollStart, flScrollEnd);

			if (flScrollLepred < 0.f)
			{
				bSmoothScrolling = bInterpolated = false;
				flScrollStart = flScrollEnd = 0.f;
				flScrollLepred = 0.f;
			...
			if (flInterolateTime + 0.125f < g_pGlobalVars->curtime)
			{
				bSmoothScrolling = bInterpolated = false;
				flScrollStart = flScrollEnd = 0.f;
			}
...
void ImGui::NewFrame()
{
	...
	if (g.TempInputTextId && g.ActiveId != g.TempInputTextId)
		g.TempInputTextId = 0;
...
static bool ImGui::BeginChildElementEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = g.CurrentWindow;
	...
	if (!border)
		g.Style.ChildBorderSize = 0.0f;
	bool ret = BeginElement(name, NULL, flags);
	g.Style.ChildBorderSize = backup_border_size;
...
bool ImGui::BeginChildElement(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	return str_id == FindRenderedTextEnd(str_id)
		? BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags)
		: BeginChildElementEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		window->ContentsRegionRect.Max.y = window->Pos.y - window->Scroll.y - window->WindowPadding.y + (window->SizeContentsExplicit.y != 0.0f ? window->SizeContentsExplicit.y : (window->Size.y - window->ScrollbarSizes.y + ImMin(window->ScrollbarSizes.x, window->WindowBorderSize)));
...
bool ImGui::BeginElement(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
			{
				...
				window->DrawList->AddLine(window->Pos, ImVec2(window->Pos.x + 20.f, window->Pos.y), GetColorU32(ImGuiCol_Border), border_size);
				...
				RenderText(ImVec2(window->Pos.x + 26.f, window->Pos.y - text_size.y * 0.5f), name, NULL, true);
				window->DrawList->AddLine(ImVec2(window->Pos.x + 32.f + text_size.x, window->Pos.y), ImVec2(window->Pos.x + window->Size.x, window->Pos.y), GetColorU32(ImGuiCol_Border), border_size);
			}
		}
