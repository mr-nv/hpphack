...
ImFont* g_FontProggyClean = nullptr;
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data, bool IsInGame)
{
    ...
    if (!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if (g_pd3dDevice->CreateIndexBuffer(g_IndexBufferSize * sizeof(ImDrawIdx), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, sizeof(ImDrawIdx) == 2 ? 
			D3DFMT_INDEX16 : D3DFMT_INDEX32, D3DPOOL_DEFAULT, &g_pIB, NULL) < 0)
            return;
    }

    // Backup the DX9 state
    IDirect3DStateBlock9* d3d9_state_block = NULL;
    if (g_pd3dDevice->CreateStateBlock(IsInGame ? D3DSBT_PIXELSTATE : D3DSBT_ALL, &d3d9_state_block) < 0)
        return;
...
bool ImGui_ImplDX9_Init(IDirect3DDevice9* device)
{
	...
	style.FramePadding = ImVec2(4, 0.5f);
	...
	g_FontProggyClean = io.Fonts->AddFontFromMemoryCompressedBase85TTF(proggy_clean_ttf_compressed_data_base85, 13);
