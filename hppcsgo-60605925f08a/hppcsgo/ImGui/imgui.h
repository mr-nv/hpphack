...
struct ImVec4
{
	...
	float& operator[](size_t idx) { if (idx >= 0 && idx < 4) return ((float*)this)[idx]; return ((float*)this)[0]; }
...
namespace ImGui
{
    ...
    IMGUI_API void          Render(ImDrawList* our_list = nullptr);                                   // ends the ImGui frame, finalize the draw data. (Obsolete: optionally call io.RenderDrawListsFn if set. Nowadays, prefer calling your render function yourself.)
	...
	IMGUI_API bool          BeginElement(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
    IMGUI_API void          End();
	...
	IMGUI_API bool          BeginChildElement(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0);
