...
static std::map<int, const char*> g_weapon_map =
{
	/* PISTOLS */
...
static auto* GetWeaponName(int index)
{
	const auto entry = g_weapon_map.find(index);
	return entry == end(g_weapon_map) ? "unknown" : entry->second;
}

static int GetWeaponIndex(const char* name)
{
	return std::find_if(g_weapon_map.begin(), g_weapon_map.end(),
