...
static inline void LegitBot(std::string sPath)
{
	...
	for (int i = 1; i < MAX_WEAPONS; ++i)
	{
		...
		AddKey(sSection + ".aim.smooth", &cvar.legitbot[i].aim_smooth);
...
static inline void Visuals(std::string sPath)
{
	...
	AddKey("screen.log.hit_registrator", &cvar.screen_log_hit_registrator);
	AddKey("screen.log.only_my_kills", &cvar.screen_log_only_my_kills);
...
static inline void Misc(std::string sPath)
{
	...
	AddKey("backtrack.enabled", &cvar.backtrack_enabled);
	AddKey("backtrack.value", &cvar.backtrack_value);
	AddKey("backtrack.key.press", &cvar.backtrack_key_press);
	...
	AddKey("protector", &cvar.protector);
