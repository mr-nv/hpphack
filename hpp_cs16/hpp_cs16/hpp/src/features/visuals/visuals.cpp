...
static void НарисоватьСтрелочку(int куда, ImColor цвет)
{
	Vector2D screen;
...
void CVisuals::Overlay()
{
	...
	if (!g_Local->m_bIsDead && cvars::visuals.esp_other_local_aa_side_arrows && cvars::ragebot.active && cvars::ragebot.aa_enabled && cvars::ragebot.aa_stand_desync && !g_pGlobals->m_flGaitMovement)
	{
		...
		{
			НарисоватьСтрелочку(0, ImColor(100, 255, 100));
			НарисоватьСтрелочку(1, ImColor(100, 100, 100));
		}
		else {
			НарисоватьСтрелочку(0, ImColor(100, 100, 100));
			НарисоватьСтрелочку(1, ImColor(100, 255, 100));
		}
	}
}
