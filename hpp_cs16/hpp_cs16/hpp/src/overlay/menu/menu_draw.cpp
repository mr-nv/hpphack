...
void CMenu::Draw()
{
	...
	if (m_pGui->Begin("##menu", ImVec2(800, 600)))
	{
		...
		case LEGIT:
		{
			...
			m_pGui->Slider(&cvars::weapons[current_weapon_id].aim_fov, "Maximum FOV", 0.f, 180.f, u8"%.1f°", 2.5f);
			...
			m_pGui->Slider(&cvars::weapons[current_weapon_id].aim_recoil_fov, "Recoil compensation maximum FOV", 0.f, 180.f, u8"%.1f°", 2.5f);
			...
			m_pGui->Slider(&cvars::weapons[current_weapon_id].aim_psilent_angle, "Perfect silent maximum angle", 0.f, 1.f, u8"%.1f°", 1.f);
		...
		case RAGE:
		{
			...
			case RAGE_MAIN:
			{
				...
				const char* raim_remove_spread[] = { "None", "Pitch / Yaw", "Pitch / Roll", u8"Pitch / Yaw / Roll" };
				const char* raim_resolver_pitch[] = { "None", u8"Breakpoint 90°" };
				...
				m_pGui->Slider(&cvars::ragebot.raim_fov, "Maximum FOV", 0.f, 180.f, u8"%.0f°", 1.f);
				...
				{
					const char* aa_roll[] = { "None", u8"Sideways 50°", u8"Sideways 90°", u8"Sideways 180°", "Static" };
				...
				case RAGE_ANTIAIM_STAND:
				{
					...
					const char* aa_stand_yaw[] = { "None", "Backwards", u8"Sideways 90°", u8"Sideways 140°", "Static", "Local view", "Backwards desyns", };
					...
					m_pGui->Slider(&cvars::ragebot.aa_stand_yaw_static, "Yaw static", -180.f, 180.f, u8"%.0f°", 1.f);
				...
				case RAGE_ANTIAIM_MOVE:
				{
					...
					const char* aa_move_yaw[] = { "None", "Backwards", "Local view", u8"Gait sideways 120°" };
		...
		case MISC:
		{
			...
			//{
				...
				m_pGui->Slider(&cvars::misc.kb_fov, "Maximum FOV", 0.f, 180.f, u8"%.0f°", 1.f);
