...
namespace General
{
	static void DrawAim(const std::unique_ptr<CMenuGui>& m_pGui)
	{
		...
		m_pGui->Checkbox(&cvars::legitbot.aim_smooth_independence_fps, "Smooth independence fps", u8"ENG: Turns off smoothing angles dependency on FPS.\nRUS: Выключает зависимость сглаживания от FPS.");
		...
		m_pGui->Checkbox(&cvars::legitbot.aim_demochecker_bypass, "Demochecker bypass", u8"ENG: Sets only the correct angles.\nRUS: Устанавливает только правильные углы.");
	...
	static void DrawTrigger(const std::unique_ptr<CMenuGui>& m_pGui)
	{
		...
		m_pGui->Checkbox(&cvars::legitbot.trigger_accurate_traces, "Accurate traces", u8"ENG: Does not shoot if the selected hitbox overlaps an unselected hitbox.\nRUS: Не стреляет, если выбранный хитбокс перекрывает невыбранный хитбокс.");
...
void CMenu::DrawLegit()
{
	...
	//m_pGui->Checkbox(&cvars::legitbot.position_adjustment, "Position adjustment", u8"ENG: Exploit allowing you to shoot at multiple positions of the target.\nRUS: Эксплоит позволяющий стрелять в несколько позиций цели.");
	////m_pGui->PopTextColor();
	...
	//m_pGui->Slider(&cvars::weapons[current_weapon_id].aim_fov, "Maximum FOV", 0.f, 180.f, u8"%.1f°", 1.f);
	...
	//m_pGui->Slider(&cvars::weapons[current_weapon_id].aim_recoil_fov, "Recoil comp. maximum FOV", 0.f, 180.f, u8"%.1f°", 1.f);
