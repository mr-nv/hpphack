...
namespace AntiAim
{
	static void DrawGlobals(const std::unique_ptr<CMenuGui>& m_pGui)
	{
		...
		m_pGui->Combo(&cvars::ragebot.aa_roll, "Roll", aa_roll, IM_ARRAYSIZE(aa_roll), u8"ENG: It works only on special servers. (Example: hvh.hpp.ovh:27015)\nRUS: Работает только на специальных серверах (Например: hvh.hpp.ovh:27015).");
		//m_pGui->PushCondition(cvars::ragebot.aa_roll == 4);
		m_pGui->Slider(&cvars::ragebot.aa_roll_static, "Roll static", -180.f, 180.f, u8"%.1f°", 1.f);
		//m_pGui->PopCondition();
		//m_pGui->PopTextColor();
		m_pGui->Checkbox(&cvars::ragebot.aa_untrusted_checks, "Untrusted checks", u8"ENG: Use only trusted angles.\nRUS: Использовать только надежные углы.");
		//m_pGui->PopCondition();
	...
	static void DrawStand(const std::unique_ptr<CMenuGui>& m_pGui)
	{
		...
		m_pGui->Slider(&cvars::ragebot.aa_stand_yaw_static, "Yaw static", -180.f, 180.f, u8"%.1f°", 1.f);
		//m_pGui->PopCondition();
...
void CMenu::DrawRage()
{
	...
	//const char* raim_remove_spread[] = { "Pitch / Yaw", "Pitch / Roll", u8"Pitch / Yaw / Roll" };
	...
	//m_pGui->Slider(&cvars::ragebot.raim_fov, "Maximum FOV", 0.f, 180.f, u8"%.1f°", 1.f);
